use std::{cell::LazyCell, clone::Clone, mem, path::PathBuf};
...
impl Config {
    ...
    pub fn list(&mut self, current_scope: String, all: bool, scopes: bool) -> Result<LsResult> {
        if all {
            sort_servers(&mut self.servers);
            return Ok(LsResult::All(mem::take(&mut self.servers)))
        }
        if scopes {
            self.scopes.sort_unstable_keys();
            return Ok(LsResult::AllScopes(mem::take(&mut self.default), mem::take(&mut self.scopes)))
        }
        if self.scopes.contains_key(&current_scope) {
            ...
            return Ok(LsResult::Scope(current_scope, result))
        }

        sort_servers(&mut self.servers);
        Ok(LsResult::All(mem::take(&mut self.servers)))
    }

    pub fn add_server(mut self, name: String, server: Server, global: bool) -> Result<Self> {
        if global {
            add_global_server(&mut self.servers, name, server)?;
            return Ok(self)
        }
        ...
        if server.is_only_address() {
            ...
            scope_servers.insert(name, ScopedServer::Override(server));
        }
        self.servers.insert(current_scope, ServerEntry::Scope(scope_servers));
    ...
    pub fn remove(mut self, server: Option<String>, scope: Option<String>) -> Result<Self> {
        ...
        if let Some(ServerEntry::Scope(scope_servers)) = self.servers.get_mut(context.scope()) {
            if scope_servers.swap_remove(&name).is_none() {
                return Err(CliError::ServerNotFound(name.into()).into())
            }
            if scope_servers.is_empty() {
                self.servers.swap_remove(&context.into_scope());
            }
            return Ok(self)
        }
...
impl StorageProvider for Config {
    const WORK_FILE: LazyCell<Box<str>> = CONFIG_PATH;
...
impl Server {
    ...
    pub fn from_uri(uri: &mut ServerUri) -> Self {
    ...
    pub fn apply_scope(&mut self, scope: Scope) {
        ...
        *user = user.take().or(scope.user);
        *port = port.take().or(scope.port);
        *known_hosts = known_hosts.take().or(scope.known_hosts);
        *private_key = private_key.take().or(scope.private_key);
        *openssh_cert = openssh_cert.take().or(scope.openssh_cert);
    }
}
...
#[inline]
fn sort_servers(servers: &mut IndexMap<String, ServerEntry>) {
    servers.sort_unstable_keys();
    servers.values_mut().filter_map(|entry|
        if let ServerEntry::Scope(s) = entry { Some(s) }
        else { None }
    )
...
mod tests {
    ...
    fn list_flags() {
        ...
        assert!(matches!(res, Ok(LsResult::AllScopes(_, _))));
    ...
    fn list_content() {
        ...
        let res = cfg.list("".into(), true, false);
        assert!(matches!(res, Ok(LsResult::All(_))));
        if let Ok(LsResult::All(servers)) = res {
        ...
        let res = cfg.list("".into(), false, true);
        assert!(matches!(res, Ok(LsResult::AllScopes(_, _))));
        if let Ok(LsResult::AllScopes(_, scopes)) = res {
        ...
        cfg.servers.insert(scope1.clone(), ServerEntry::Scope(scoped_servers));

        let res = cfg.list(scope1, false, false);
        assert!(matches!(res, Ok(LsResult::Scope(_, _))));
        if let Ok(LsResult::Scope(_, servers)) = res {
    ...
    fn serialize_deserialize() {
        ...
        let cfg = Config {
            default: Some(Scope { user: default_user.clone(), ..Default::default()}),
            ..Default::default()
        };
        if let Ok(serialized) = StorageProvider::serialize(&cfg) {
            ...
            assert!(deserialized.is_ok_and(|c|
                c.default.is_some_and(|s| s.user == default_user))
            );
        } else {
    ...
    fn from_server_uri() {
        ...
        let srv = Server::from_uri(&mut uri);
